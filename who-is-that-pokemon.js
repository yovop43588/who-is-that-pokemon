...
function buildMarkup(pokemon, revealTime) {
...
<head>
  ...
  <style>
    html, body { margin: 0; padding: 0; font-family: sans-serif; display: flex; flex-direction: column; align-items: center; justify-content: center; height: 100vh; }
    #container { position: relative; width: 256px; height: 256px; }
    ...
    #reveal-button { margin-top: 1rem; padding: 0.5rem 1rem; font-size: 1rem; cursor: pointer; }
  </style>
</head>
<body>
...
const server = http.createServer(async (req, res) => {
  ...
  try {
    ...
    if (pathname === '/markup' && req.method === 'GET') {
      ...
      const html = buildMarkup(pokemon, cfg.revealTime);
      res.writeHead(200, { 'Content-Type': 'text/html' });
      res.end(html);
      return;
    }
    ...
    // installation and webhook endpoints (no‑ops in this minimal version)
